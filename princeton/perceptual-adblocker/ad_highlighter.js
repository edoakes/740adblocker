...
function findAds(container, firstRun) {
  if (container.find) {
    findNewsfeedAds(container, firstRun, newsfeedFunction);
    findSidebarAds(container, firstRun, sidebarFunction);
  }
}
...
function newsfeedFunction(container, matchingText, deepestOnly) {
  chrome.storage.sync.get({
    fbnewsfeed: 0.5,
    remove: false,
  }, function(items) {
      if (100*Math.random() > parseInt(items.fbnewsfeed)) {
        return coverContainer(container, "NEWSFEED AD", matchingText, deepestOnly, true, false, null, items.remove);
...
function sidebarFunction(container, matchingText, deepestOnly) {
  chrome.storage.sync.get({
    fbsidebar: 0.5,
    remove: false,
  }, function(items) {
      if (100*Math.random() > parseInt(items.fbsidebar)) {
        return coverContainer(container, "SIDEBAR AD", matchingText, deepestOnly, true, false, null, items.remove);
      }
...
var observer = new MutationObserver(function(mutations) {
  mutations.forEach(function(mutation) {
    ...
    if (mutation.addedNodes.length > 0) {
      findAds($(mutation.addedNodes), false);
    }
  });
});
